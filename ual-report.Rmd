...
majority o fthe libraries investigated here. At the rare end, support for 
...
expenditures, may influence the total number of data services offered. Data 
from Washington State University were excluded from analyses: this library 
offered the fewest surveyed data services (4), had the second-lowest 
salary and wage expenditures (approximately $6.2 million), and was classified as 
an outlier based on Cook's distance. In a regression analysis of the remaining 
24 institutions, salary and wage expenditures was significantly correlated with 
the total number of services a library offered, with each increase of 
approximately $1 million in salary and wage expenditures leading to one 
additional data service provided (_p_ = 0.03; dotted line in Figure 2). 
However, this relationship may be driven in large part by a single library: 
Stanford University offered the most data services (15) and had the highest 
resourcing ($47 million total salary and wage expenditures). When we removed 
Stanford University from analysis (the library has 46% higher resourcing than 
the next-highest resourced library, at UC Berkeley), the relationship between 
salary and wage expenditures and the number of services is no longer 
significant (_p_ = 0.21; solid line in Figure 2).

![Total number of services offered as a function of library's total annual 
salaries and wages. Dotted line shows regression excluding outlier Washington 
State University (lower left corner); solid line shows regression excluding 
Washington State University and Stanford University (upper right corner); red 
...
provides more services than expected (i.e. it is above the regression lines in 
Figure 2).
...
expertise that may not be common in library personnel. These unsupported 
services do highlight a limitation of our approach: statistical consulting and 
support for version control are both provided through the R programming 
sessions, but are not explicitly mentioned on UAL web sites. This is largely 
due to the limited capacity of the Data Cooperative: advertising support for 
these services, especially statistical consulting, would be disingenuous, given 
the high demand for such services on campus and limited number of personnel who 
can support such services.
